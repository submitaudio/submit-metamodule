...
// The VCV Rack version keeps its own eight-voice configuration.
static constexpr int CLANG_VOICES = 3;
...
struct ClangVoice {
	...
	float noiseGain = 1.f;
	float noiseCharacterGain = 1.f;
	float brightTrim = 1.f;
	float noiseFollowBase = 0.f;
	float localEdgeCache = 0.f;
	float hpFeedbackCache = 0.f;
	float rawClickThresholdCache = 0.f;
	float bodyDecayCoeff = 1.f;
	float noiseDecayCoeff = 1.f;
	float clickDecayCoeff = 1.f;
	float subDecayCoeff = 1.f;
	float decayCache = -1.f;
	float bodyCache = -1.f;
	float sampleTimeCache = -1.f;
	float transientEnv = 0.f;
	int noiseColor = 0;
	int engine = 0;
	int model = 0;
	int sound = 0;
...
struct Clang : Module {
	...
	float triggerLightDecay = 1.f;
	float pitchEnvDecay = 1.f;
	float ghostEnvDecay = 1.f;
	float transientEnvDecay = 1.f;
	float globalDecayCurveCache = -1.f;
	float globalSampleTimeCache = -1.f;
	int lightUpdateCounter = 0;
	static constexpr int LIGHT_UPDATE_DIV = 32;
	int lastEngine = 0;
	...
	size_t get_display_text(int displayId, std::span<char> text) override {
		...
		int model = clamp((int)std::round(params[MODEL_PARAM].getValue()), 0, CLANG_MODELS - 1);
		int sound = clamp((int)std::round(params[SOUND_PARAM].getValue()), 0, CLANG_SOUNDS - 1);
		if (engine == 1)
			model = clangFmModelOrder[model];

		std::string displayText = engine == 0
		...
		size_t charsToCopy = std::min(text.size(), displayText.size());
		std::copy(displayText.begin(), displayText.begin() + charsToCopy, text.begin());
		return charsToCopy;
	}
	...
	static float randomToWhite(uint32_t value) {
		return (float)(int32_t)value / 2147483648.f;
	}

	float nextWhite() {
		return randomToWhite(nextRandom());
	}
	...
	void triggerVoice(int engine, int model, int sound, float sampleTime) {
		...
		voice.soundPos = (float)sound / 7.f;
		voice.noiseColor = (engine * 13 + model * 5 + sound * 3) & 15;
		voice.noiseCharacterGain = 1.35f + 0.18f * (float)model + 0.10f * (float)sound;
		voice.clickPresence = 0.f;
		if (engine == 0 && model == 0)
			voice.clickPresence = 0.90f;
		else if (engine == 0 && model == 3 && (sound == 0 || sound == 2 || sound == 5 || sound == 7))
			voice.clickPresence = 0.52f;
		else if (engine == 0 && model == 5 && (sound == 0 || sound == 6))
			voice.clickPresence = 0.34f;
		else if (engine == 0 && model == 6 && (sound == 2 || sound == 6))
			voice.clickPresence = 0.28f;
		else if (engine == 0 && model == 7 && (sound == 0 || sound == 6))
			voice.clickPresence = 0.24f;
		else if (engine == 1 && model == 0)
			voice.clickPresence = 0.48f;
		else if (engine == 1 && model == 3)
			voice.clickPresence = 0.42f;
		voice.age = 0.f;
		voice.transientEnv = 1.f;
		voice.decayCache = -1.f;
		voice.bodyCache = -1.f;
		voice.sampleTimeCache = -1.f;
		voice.attackEnv = 0.f;
		...
		voice.attackStep = sampleTime / voice.attackTime;
		...
		voice.noiseGain = 0.42f + 0.88f * clamp(voice.noiseLevel, 0.f, 1.f);
		voice.brightTrim = 1.f - 0.18f * clamp(voice.bright, 0.f, 1.f);
		voice.noiseFollowBase = 0.10f + 0.30f * voice.bright + 0.02f * (float)model;
	...
	void process(const ProcessArgs& args) override {
		...
		if (trigIn.process(inputs[TRIG_INPUT].getVoltage(), 0.1f, 2.f)) {
			triggerVoice(engine, model, sound, args.sampleTime);
			triggerLightEnv = 1.f;
		}
		if (globalSampleTimeCache != args.sampleTime) {
			globalSampleTimeCache = args.sampleTime;
			globalDecayCurveCache = -1.f;
			triggerLightDecay = std::exp(-args.sampleTime / 0.080f);
			transientEnvDecay = std::exp(-args.sampleTime / 0.018f);
			for (int v = 0; v < CLANG_VOICES; v++)
				voices[v].attackStep = args.sampleTime / voices[v].attackTime;
		}
		triggerLightEnv *= triggerLightDecay;
		if (++lightUpdateCounter >= LIGHT_UPDATE_DIV) {
		...
		if (globalDecayCurveCache != decayCurve) {
			globalDecayCurveCache = decayCurve;
		...
		for (int v = 0; v < CLANG_VOICES; v++) {
			...
			if (envelopeEnergy < 1e-5f) {
				if (voice.ghostDelay > 0.f && voice.ghostAmt > 1e-5f) {
					...
					voice.transientEnv *= transientEnvDecay;
			...
			voice.attackEnv = std::min(1.f, voice.attackEnv + voice.attackStep);
			if (voice.age > 4.f) {
			...
			if (voice.decayCache != decay || voice.bodyCache != bodyControl || voice.sampleTimeCache != args.sampleTime) {
			...
			voice.bodyDecayCoeff = std::exp(-args.sampleTime / std::max(0.0002f, bodyMs / 1000.f));
			voice.noiseDecayCoeff = std::exp(-args.sampleTime / std::max(0.0002f, noiseMs / 1000.f));
			voice.clickDecayCoeff = std::exp(-args.sampleTime / std::max(0.00005f, clickMs / 1000.f));
			float subMs = clamp((18.f + 170.f * decayCurve) * subContour[soundV] * (0.55f + 0.80f * damp) * tailCurve, 0.20f, 2400.f);
			voice.subDecayCoeff = std::exp(-args.sampleTime / std::max(0.0002f, subMs / 1000.f));
			voice.localEdgeCache = clamp(edge * 0.72f + bodyControl * 0.18f + voice.foldAmt * 0.38f, 0.f, 1.f);
			voice.hpFeedbackCache = clamp(0.58f + 0.30f * damp - 0.16f * voice.bright, 0.30f, 0.96f);
			voice.rawClickThresholdCache = 0.35f + 0.45f * damp - 0.18f * voice.bright;
			voice.decayCache = decay;
			voice.bodyCache = bodyControl;
			voice.sampleTimeCache = args.sampleTime;
			}

			voice.bodyEnv *= voice.bodyDecayCoeff;
			voice.noiseEnv *= voice.noiseDecayCoeff;
			voice.clickEnv *= voice.clickDecayCoeff;
			voice.subEnv *= voice.subDecayCoeff;
			...
			float bend = voice.pitchBend == 0.f ? 1.f : std::pow(2.f, voice.pitchEnv * voice.pitchBend);
			float motionCycle = 0.f;
			if (motionAmount > 0.f)
				motionCycle = std::sin(voice.phaseSub * (0.85f + 5.25f * motionAmount) + voice.phase1 * 0.07f) * motionAmount;
			...
			float follow = voice.noiseFollowBase + 0.62f * motion;
			voice.noiseTone += clamp(follow, 0.05f, 0.98f) * (white - voice.noiseTone);

			float hpIn = voice.noiseTone;
			float hp = hpIn - voice.hpX + voice.hpFeedbackCache * voice.hpY;
			voice.hpX = hpIn;
			voice.hpY = hp;

			// Preserve the original random stream even when a colour is not needed.
			uint32_t burstRandom = nextRandom();
			uint32_t spitRandom = nextRandom();
			uint32_t ghostRandom = nextRandom();

			// MetaModule only evaluates a sound building block when the selected
			// model actually uses it. Cached lambdas preserve the exact formulas.
			...
			CLANG_LAZY(s1, std::sin(voice.phase1));
			CLANG_LAZY(phase2Base, std::sin(voice.phase2));
			CLANG_LAZY(burstWhite, randomToWhite(burstRandom));
			CLANG_LAZY(spitWhite, randomToWhite(spitRandom));
			CLANG_LAZY(ghostWhite, randomToWhite(ghostRandom));
			float noisePhase = noiseAmount * (0.15f + 0.85f * ((soundV == 3 || soundV == 4 || soundV == 7) ? 1.f : 0.f));
			CLANG_LAZY(motionPhase, motionAmount > 0.f
				? std::sin(voice.phaseSub * (1.1f + 3.9f * motionAmount) + voice.phase2 * 0.11f) * motionAmount * 0.95f
				: 0.f);
			CLANG_LAZY(s2, motionAmount > 0.f
				? std::sin(voice.phase2 + motionPhase() + voice.noiseTone * motionAmount * noisePhase * (1.5f + 3.f * voice.soundPos))
				: phase2Base());
			...
			float localEdge = voice.localEdgeCache;
			...
			if (noiseAmount > 0.f) {
				...
				noise = shapedNoise * voice.noiseEnv * voice.noiseGain * noiseAmount;
			}
			// Click is an intentional material property, never a universal attack.
			// Routes without click presence avoid both click oscillator calculations.
			CLANG_LAZY(rawClick, voice.clickPresence > 0.f
				? (white > voice.rawClickThresholdCache ? 1.f : -1.f) * voice.clickEnv * voice.clickLevel * voice.clickPresence
				: 0.f);
			CLANG_LAZY(tonalClick, voice.clickPresence > 0.f
				? (s1() * 0.58f + std::sin(voice.phase1 * 2.317f + voice.phase2 * 0.13f) * 0.42f)
					* voice.clickEnv * voice.clickLevel * voice.clickPresence
				: 0.f);
			CLANG_LAZY(click, tonalClick() * (1.f - 0.42f * noiseAmount)
				+ (noiseAmount > 0.f ? rawClick() * noiseAmount * 0.72f : 0.f));
			...
			float noiseCharacter = noise * voice.noiseCharacterGain;
			...
			soundOut *= bodyMix;
			float transientWeight = voice.transientEnv;
			float levelTrim = voice.bodyTrim + (voice.transientTrim - voice.bodyTrim) * transientWeight;
			soundOut *= voice.brightTrim * voice.outputTrim * levelTrim;
		...
		// Keep dense overlaps punchy instead of letting three tails saturate
		// into a continuous drone when Decay is high.
		if (activeEnergy > 1.f)
			out /= std::sqrt(activeEnergy);
		out = dcBlock(out);
...
struct ClangDisplay : Widget {
	...
	void draw(const DrawArgs& args) override {
		...
		nvgText(args.vg, box.size.x / 2.f + 3.f, box.size.y / 2.f + 3.f, txt.c_str(), NULL);
	}
};
...
struct ClangWidget : ModuleWidget {
	ClangWidget(Clang* module) {
		...
		addRoundKnob(Vec(34.362f, 108.772f), Clang::MODEL_PARAM);
		addParam(createParam<CKSS>(Vec(115.317f, 103.096f), module, Clang::ENGINE_PARAM));
		addRoundKnob(Vec(83.121f, 108.772f), Clang::SOUND_PARAM);
		addRoundKnob(Vec(34.362f, 216.627f), Clang::TUNE_PARAM);
		addRoundKnob(Vec(83.191f, 216.627f), Clang::DECAY_PARAM);
		addRoundKnob(Vec(132.02f, 216.627f), Clang::NOISE_PARAM);
		addRoundKnob(Vec(34.362f, 285.601f), Clang::MOTION_PARAM);
		addRoundKnob(Vec(83.191f, 285.601f), Clang::BODY_PARAM);
		addRoundKnob(Vec(132.02f, 285.601f), Clang::VARIATION_PARAM);
