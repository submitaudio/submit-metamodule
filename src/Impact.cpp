...
struct Impact : Module {
    ...
    // Snelle sin approximatie
    static inline float fast_sin(float x) {
        x = x - floorf(x);
        float y = x * 2.f - 1.f;
        return y * (1.273239f - 0.405285f * y * y);
    }
    ...
    void process(const ProcessArgs& args) override {
        ...
        if (inputs[VOCT_INPUT].isConnected()) {
            ...
            baseFreq = clamp(baseFreq * std::exp2(cv), 10.f, 2000.f);
        }
        ...
        float punchCurved = punchKnob * punchKnob * std::sqrt(punchKnob);  // pow(x,1.8) approximatie
        float pitchMult   = 1.f;
        if (active) {
        ...
        if (active) {
            ...
            float decayRate = std::exp((1.f - decayKnob) * 2.3f * 2.302585f) * 0.4f + 0.4f;  // pow(10,x) = exp(x*ln10)
            ...
            if (mode == 0) {
                ...
                float modSig   = fast_sin(fmPhase[0]) * fmIndex;
                float carPhase = phase[0] + modSig * 0.1f;
                float carrier  = fast_sin(carPhase);
                sum = carrier * masterEnv;
                sum = fold(sum, foldKnob);
                sum = clamp(sum, -1.f, 1.f);
            }

            float noiseDecayRate = std::exp((1.f - nlenKnob) * 3.0f * 2.302585f) * 0.05f + 0.003f;
            float noiseEnv  = std::exp(-t * noiseDecayRate);
            float noiseAmt  = noiseKnob * std::sqrt(noiseKnob) * 1.4f;  // pow(x,1.5) = x*sqrt(x)
